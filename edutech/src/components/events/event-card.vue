  ...
  <b-card-body style="max-height: 350px; overflow-y: auto;">
    <b-row>
      <b-col :md="isMinimized? '3' :'2'" sm="12" class="text-center">
        <img v-if="task.isGoogle" :src="task.icon" class="left" height="75" width="75"/>
        <b-img width="75" height="75" v-if="task.icon&&!task.isGoogle" :src="'http://localhost:5000/api/file/'+task.icon"></b-img>
      </b-col>
      <b-col :md="isMinimized? '8' :'6'" sm="12">
        <b-card-text class="text-dark" v-if="task.name">
      ...
      <b-col md="4" sm="12" class="text-center" v-if="!isMinimized">
        <b-button v-if="false" variant="outline-info">+ ADD TO CALENDAR</b-button>
      </b-col>
    </b-row>
  </b-card-body>
</template>
